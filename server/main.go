...
func (s *GraphContext) CreateGraph(origin *pb.Origin, stream pb.Graph_CreateGraphServer) error {
	edges, lastNodes := matrix.CreateSimpleGraph(relationMap, int(origin.Degree), strings.ToUpper(origin.Guid))
...
func main() {
	f, _ := os.Open("D:\\Projects\\Golang\\bin\\sample\\sample_relationships.csv")
